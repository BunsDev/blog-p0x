...
如果有两个 position 中的流动性相等，例如 `L = 500`，并且这两个 position 同时引用了一个 tick，其中一个为 lower tick ，另一个为 upper tick，那么对于这个 tick `liquidityNet = 0`。此时我们就需要一种机制来判断此 tick 是否仍然在被引用。使用 `liquidityGross` 记录流动性的增值（不考虑 lower/upper），我们可以通过流动性变化前后 `liquidityGross` 是否等于0 来判断这个 tick 是否仍被引用。
