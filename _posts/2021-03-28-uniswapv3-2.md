...
$$
\begin{cases}
i = n_1 \times 1 + n_2 \times 2 + n_3 \times 4 + n_4 \times 8 + .... \\
n_i \in \{0, 1\}
\end{cases}
$$
...
$$
\begin{cases}
\sqrt P = {\sqrt {1.0001}}^i = n_1{\sqrt {1.0001}}^1 \times n_2{\sqrt {1.0001}}^2 \times n_3{\sqrt {1.0001}}^3 \times ... \\
n_i \in \{0, 1\}
\end{cases}
$$

因为 $i \in (-887272, 887272)$，只需要 20 位二进制数可以保存其值。我们可以预先算出 ${\sqrt {1.0001}}^1,\ {\sqrt {1.0001}}^2,\ {\sqrt {1.0001}}^3\ ...\ {\sqrt {1.0001}}^{20}$ 的值，然后将 $i$ 值每一位的值求出，带入上面的计算公式就可以算出 $\sqrt P_i$ 的值。

实际上，这段代码在上面的算法之上还进行了优化：

- 因为 $\sqrt P_{-i} = \frac 1{\sqrt P_{i}}$，所以当 $i$ 为负数时可以先将其取反转为正数计算
- 当 $i$ 的值为正数时，计算的结果可能会很大，中间计算的步骤可能会溢出，导致丢失最大位的精度，所以实际计算的是 $i$ 为负数时的值，因为这样丢失的是负数最小位的精度（因为它使用了 `Q128.128` 定点数）。即上面代码的那些魔数分别为 $\frac 1{\sqrt {1.0001}^1},\ \frac 1{\sqrt {1.0001}^2},\ \frac 1{\sqrt {1.0001}^3},\ ...,\ \frac 1{\sqrt {1.0001}^{20}}$ 的值
