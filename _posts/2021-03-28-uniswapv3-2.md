...
这里先通过 `keccak256(abi.encode(token0, token1, fee)` 将 `token0`, `token1`, `fee` 作为输入，得到一个哈希值，并将其作为 `salt` 来创建合约。因为制定了 `salt`, solidity 会使用 EVM 的 `CREATE2` 指令来创建合约。使用 `CREATE2` 指令的好处是，只要合约的 `bytecode` 及 `salt` 不变，那么创建出来的地址也将不变。
...
为什么不直接使用参数传递来对新合约的状态变量赋值呢。这是因为 `CREATE2` 不会将合约构造函数的参数作为 salt，如果创建合约时指定参数，还需要再将这些参数求 hash 后作为 salt 传递给 CREATE2. 这样会让参数重复出现 2 次，Uniswap 的开发人员选择了这样的实现方式来避免这个问题。
...
struct AddLiquidityParams {
    ...
    int24 tickUpper;    // 流动性的价格上线（以 token1 计价），这里传入的是 tick index
...
如前所述，因为合约的参数接受的是流动性 $L$ 的值，我们需要在链下通过用户愿意提供流动性包含的 token 数，计算出 $L$。

假设用户提供流动性的价格范围是：$[P_a, \ P_b]\ (P_a < P_b)$，可以分成三种情况来计算流动性 $L$ 的值：
...
$$
L = \frac {\Delta x}{\frac 1{\sqrt {P_{b}}} - \frac 1{\sqrt {P_{a}}}}
$$
...
$$
L = \frac {\Delta x}{\frac 1{\sqrt {P_{b}}} - \frac 1{\sqrt {P_{c}}}} = \frac {\Delta y}{\sqrt {P_{c} - {P_{a}}}}
$$

### 回调函数

使用回调函数原因是，将 `Position` 的 owner 和实际流动性 token 支付者解耦。这样可以让中间合约来管理用户的流动性，并将流动性 token 化。关于 token 化，Uniswap v3 默认实现了 ERC721 token（因为即使是同一个池子，流动性之间差异也也很大）。其实对于单个池子来说，如果确定好流动性的价格边界，它也可以是 ERC20，这对挖矿合约来说具有参考意义（即挖矿合约需要自己管理一个 ERC20）。
...
我们再来看 tick 相关的管理，在 `UniswapV3Pool` 合约中又两个状态变量记录了 tick 相关的信息：
...
`liquidityNet` 表示当价格从左至右讲过此 tick 时整体流动性需要变化的净值。在单个流动性中，对于 lower tick 来说，它的值为正，对于 upper tick 来说它的值为 负。
如果两个 position 中的流动性相等，例如 `L = 500`，而对于这个 tick `liquidityNet = 0`，此时我们需要一种机制来判断此 tick 是否还被引用。使用 `liquidityGross` 记录流动性的增值（不考虑 lower/upper），我们可以通过流动性变化前后 `liquidityGross` 是否等于0 来判断这个 tick 是否仍被引用。

当价格变动导致 $tick_{current}$ 越过一个 position 的 lower/upper tick 时，我们将需要根据 tick 中记录的值来更新池子的总体流动性。假设 position 的流动性值为 $\Delta L$，会有以下四种情况：

1. token0 价格上升，即从左至右越过一个 lower tick 时， $L=L_{current} + \Delta L$
2. token0 价格上升，即从左至右越过一个 upper tick 时， $L=L_{current} - \Delta L$
3. token0 价格下降，即从右至左越过一个 lower tick 时， $L=L_{current} + \Delta L$
...
- 不需要初始化，因为访问 map 中不存在的 key 默认值就是 0
- 通过对位图的每个 word(uint256) 简历索引来管理位图，即访问路径为 word index -> word -> tick bit
...
代码将计算的过程封装在了 `SqrtPriceMath` 库中，`getAmount0Delta` 和 `getAmount1Delta` 分别对应公式 $\Delta x = \Delta {\frac 1{\sqrt P}} \cdot L$ 和 $\Delta y = \Delta \sqrt P \cdot L$. 这两个函数中主要是封装了很多数学计算，Uniswap v3 参考[这里](https://xn--2-umb.com/21/muldiv/index.html)实现了一个精度较高的 $\frac {a \cdot b}c$ 的算法，封装在 `FullMath` 库中。
...
$$
\sqrt P = {\sqrt {1.10001}}^i = n_1{\sqrt {1.10001}}^1 \cdot n_2{\sqrt {1.10001}}^2 \cdot n_3{\sqrt {1.10001}}^3 \cdot ...
$$

因为 $i \in (-887272, 887272)$，只需要 20 位二进制数可以保存其值。我们可以预先算出 ${\sqrt {1.10001}}^1,\ {\sqrt {1.10001}}^2,\ {\sqrt {1.10001}}^3\ ...\ {\sqrt {1.10001}}^{20}$ 的值，然后将 $i$ 值每一位的值求出，带入上面的计算公式就可以算出 $\sqrt P_i$ 的值。

实际上，这段代码在上面的算法之上还进行了优化：

- 因为 $\sqrt P_{-i} = \frac 1{\sqrt P_{i}}$，所以当 $i$ 为负数时可以先将其取反转为正数计算
- 当 $i$ 的值为正数时，计算的结果可能会很大，中间计算的步骤可能会溢出，导致丢失最大位的精度，所以实际计算的是 $i$ 为负数时的值，因为这样丢失的是负数最小位的精度。即上面代码的那些魔数分别为 $\frac 1{\sqrt {1.10001}^1},\ \frac 1{\sqrt {1.10001}^2},\ \frac 1{\sqrt {1.10001}^3},\ ...,\ \frac 1{\sqrt {1.10001}^{20}}$ 的值
- 这里的计算使用了 `Q128.128` 精度的定点数，实际上这些魔数的值都向右移动 128 位
- 最后，当输入是正数时，我们需要计算 $\sqrt P_{i} = \frac 1{\sqrt P_{-i}}$，即 $\sqrt P_{i}$ 的倒数，这里因为使用了 `Q128.128` 精度的定点数，即计算的过程是： $\frac {1<<128}{\sqrt P_{-i}} << 128 = \frac {1<<256}{\sqrt P_{-i}}$，`1<<256` 可以使用 `type(uint256).max` 取近似值来表示
...
可以看到这个函数主要是将用户的 Position 保存起来，并给用户铸造 NFT token，代表其所持有的流动性。至此提供流动性的步骤就完成了。移除流动性就是上述操作的逆操作，本文不再赘述了。
