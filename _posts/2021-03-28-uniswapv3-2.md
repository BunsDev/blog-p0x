...
- 当 $i$ 的值为正数时，计算的结果可能会很大，中间计算的步骤可能会溢出，导致丢失最大位的精度，所以实际计算的是 $i$ 为负数时的值，因为这样丢失的是负数最小位的精度（因为它使用了 `Q128.128` 定点数）。即上面代码的那些魔数分别为 $\frac 1{\sqrt {1.10001}^1},\ \frac 1{\sqrt {1.10001}^2},\ \frac 1{\sqrt {1.10001}^3},\ ...,\ \frac 1{\sqrt {1.10001}^{20}}$ 的值
...
这里顺带提一句，在交易计算中会需要进行上述计算的逆计算，即给定 $\sqrt P$，需要计算出对应的 tick index. 即 $log_{\sqrt 1.0001}{\sqrt P}$ 的计算。代码中为：`TickMath.getTickAtSqrtRatio`，关于这个函数的实现，可以参考我的这篇文章：[Solidity 中的对数计算](/logarithm-in-solidity/)。
