...
1. 当用户保证率不够时，协议就需要清算用户的一部分资产，如果用户资产太多，那么意味着将产生大量的待清算资产。
2. 清算者通常使用闪电贷的方式来提高资金利用率，这需要在清算完成后立即将清算者的资产卖掉，来偿还闪电贷，这样就会对现货市场产生大量的抛压。
...
关于第 4 点中提高的协议坏账风险，这里有一个实际发生的例子：[How AAVE's $1.6 Million Bad Debt Was Created](https://eigenphi.substack.com/p/how-aaves-16-million-bad-debt-was-333)。

一个大户 Avi（Mango 协议的攻击者）在 AAVE 中抵押了大量资产借出 CRV，想通过售卖 CRV 借机清算 Curve 创始人 Michael 的仓位（Michael 在 AAVE 上抵押 CRV 借出了大量 USDC），Michael 随即在当天公开了 Curve Stablecoin 的白皮书作为回应（意思是我们还有很多 alpha 呢）。由于散户情绪被点燃，纷纷主动买入 CRV 拉高价格，导致 Avi 在 AAVE 的仓位被清算，而 Avi 的仓位太大，整个清算过程持续了几十分钟，由于清算不够及时，最终 AAVE 产生 $1.6m 的坏账。

在这个例子中，AAVE 作为借贷平台对风险控制不得当，最终产生了坏账。并且，无论 CRV 价格涨跌，被清算的人是 Michael 或者 Avi，由于仓位太大，AAVE 都很难逃脱坏账的结果。
...
LLAMMA 使用了类似 Uniswap V3 的设计，但是对其进行了大量的修改，例如 LLAMMA 也是用区间来划分流动性，可以向 AMM 的某一个区间提供流动性。

LLAMMA 和 Uniswap V3 很大的一个不同时 AMM Pool 中 2 个 token 余额的随价格变化的模式， 以 ETH/USD 为例：
...
在 LLAMMA 中（上图中虚线下半部分），用户抵押 ETH 借出 crvUSD，用户的 ETH 将被加入到 LLAMMA 中的某一个区间（这个区间的上下界价格就是开始/结束清算的价格）来提供流动性，Curve 中将其称作 band。在最开始，价格会大于 band 的 Liquidation begin price，区间中流动性全部为 ETH. 当价格下跌，区间中的 ETH 开始被转换成 crvUSD，当价格跌至区间下界价格时，用户的资产被全部转换成 crvUSD.

可以发现在 LLAMMA 中，一个区间内 token 余额变化情况刚好和 Uniswap 相反。这样就能实现清算用户资产的目的：当处于高价时，用户的资产 ETH 原封不动的放在流动性内，当价格下跌至用户流动性区间上限价格时，开始将用户的 ETH 转换成 crvUSD，当价格下跌至区间下限价格时，用户的所有 ETH 都被转换成了 crvUSD，我们假设转换完成后用户得到的 crvUSD 大于用户的债务，那么协议将不会有坏账风险，并且整个过程不需要传统借贷协议中清算者角色的介入。

这么做的好处是：

- AMM 自带流动性，减小了对外部流动性的依赖（但并不是完全不依赖，原因后文会解释）
- 不需要传统借贷协议的清算者（但在某些极端情况仍然会发生清算，具体会在后文解释），整个清算过程是伴随价格下跌，AMM 内余额的 rebalance 来完成的
- 清算是持续性的，而非阶段性的，这样用户的损失会更小
- 如果价格反弹，crvUSD 会被转换成 ETH，用户的资产被买回，这样就减少了用户的损失（但也即使价格还原，也很难做到完全无损失，原因仍在后文解释）
...
上图横轴 $p_o$ 表示 ETH 的外部价格，纵轴为 AMM 内部价格。蓝色线为一个斜率为 1 的直线，表示 AMM 内部价格追随外部价格变动。$p_{cu}$ 和 $p_{cd}$ 分别表示 AMM 中某个区间的上下界价格，$p_↑$ 和 $p_↓$ 分别表示外部价格坐标中选定的两个价格。
...
- 当外部价格 $p_o$ 大于或等于 $p_↑$ 时，区间的下界价格 $p_{cd} >= p_o$，这就对应了 Uniswap V3 中区间处于价外，且外部价格小于区间下界的情景，和 Uniswap V3 一样，这个区间中将全部为 ETH
- 当外部价格 $p_o$ 小于或等于 $p_↓$ 时，区间的上界价格 $p_{cu} <= p_o$，这就对应了 Uniswap V3 中区间处于价外，且外部价格大于区间上界的情景，那么和 Uniswap V3 也一样，这个区间中将全部为 crvUSD

这样 AMM 的区间就能够在 $p_o$ 在 ，$p_↑$ 和 $p_↓$ 之间变动时，区间内部余额变化和 Uniswap V3 刚好相反。 假设初始状态下 $p_o > p_↑$，随着 $p_o$ 下降到 $p_↑$，用户区间内的 ETH 开始被转换成 crvUSD，直至 $p_o = p_↓$ 用户的额资产全部被转换成 crvUSD.
...
其中 A 为一个大于 1 的常数。A 越大则外部价格坐标上的边界价格之间比例越小，分布越密集。这个设计很像 Uniswap 中 tick 的设计，但是要注意的是，这里的 $p_↑$ 和 $p_↓$ 都是指的外部价格，而不是 AMM 内部价格。
...
其中 x 表示 crvUSD，y 表示 ETH. 和 Uniswap V3 一样，AMM 的价格公式为：
...
公式中的 $y_0$ 表示当 $p_o = p_↑$ 时，区间内 ETH 的数量，此时 $y=y_0$, $x=0$，这里我们可以先假定 $y_0$ 为一个常数（实际上 $y_0$ 并不是一个常数，但这里可以先忽略它的改变），公式中的 $p_↑$ 和 A 也是常数。 Michael 并没有给出这两个设计的过程。但是我们可以从看出这样设计的目的：

> 随着外部价格 $p_o$ 上涨，f 以指数级增长，而 g 变小，反之 $p_o$ 下跌，f 以指数级减小，g 则会变大

我们再结合 AMM 的价格公式 $p = \frac{f + x}{g + y}$，可以看出当外部价格 $p_o$ 上涨时，LLAMMA 内部价格 $p$ 也会上涨，并且上涨速度会远高于外部价格上涨速度（分子变小而分母指数级增大），而当外部价格 $p_o$ 下跌时，LLAMMA 内部价格 $p$ 则以更快速度下跌。

这样 LLAMMA 就实现了第一个特性：**即使不发生交易，LLAMMA 内部价格也会随着外部价格自动改变，且变化速度更快**。

通过这个特性，LLAMMA 就能实现和 Uniswap V3 相反的特性，例如当 ETH 外部价格下跌时：

- Uniswap 内部价格不会改变，**套利者**会在 Uniswap 中卖出价格相对较高的 ETH，因此 Uniswap Pool 的 ETH 增加而 USD 减少
- LLAMMA 内部价格会下跌的更快，**套利者**会在 LLAMMA 中买入价格相对较低的 ETH，因此 LLAMMA Pool 中的 ETH 减少而 USD 增加

可以看出，价格下跌时，LLAMMA 通过制造价差的方式，吸引套利者介入，套利者从 LLAMMA 中买入 ETH 的行为其实就是在清算用户的资产，而套利者的获利即为买入的价差。

同样的道理，当价格下跌后，又反弹时，LLAMMA 通过价差，让套利者将 ETH 卖出到 LLAMMA 池子中，帮助用户买回了它的资产。**但是这里将很大概率无法买回用户所有的原始资产**，原因是 LLAMMA 主动改变价格的行为导致的 Path Dependence 问题（参考[这条 Tweet](https://twitter.com/danrobinson/status/1595090420740112385)），简单说就是 LLAMMA 一直在让利给套利者，这将造成用户产生损失，因此即使价格还原 LLAMMA 状态也很难还原。

另值需要注意，由于 LLAMMA 内部价格变化与外部价格呈的指数级关系，**如果套利者的介入不够及时，或者外部价格变化幅度过大，将导致 LLAMMA 和外部价格产生比较大的价差，增大用户的损失**。

### Swap 计算

当 $p_o = p_↑$ 时，区间内 ETH 的数量，此时 $y=y_0$, $x=0$，把 $f$ 和 $g$ 带入恒等式得到：

$$
I = p_oA^2y_0^2
$$

然后把这个恒等式推广到 $p_o$ 为 $p_↑ ~ p_↓$ 任意价格时：
...
在任意时刻，我们可以知道 AMM 中 x, y 余额，外部价格 $p_o$ 以及上限价格 $p_↑$，就可以求出此方程中的 $y_0$. 在 Curve 合约中，求解 $y_0$ 的代码对应为 `AMM.get_y0()` 函数：

![get y0](../img/in-post/curve-stablecoin/get-y0.png)

求出 $y_0$ 之后，就可以用上面的公式来计算交易者在 AMM 中 swap 的 output amount，在 Curve 合约中，计算交易 output amount 的代码对应为 `AMM.calc_swap_out()` 函数，这是一个非常长的函数，为了节约篇幅，这里不再展示具体代码实现。

简单描述一次 x -> y 的 swap 的计算过程：
...
用户得到的 y 数量即为所有 band 计算结果的总和。如果用户的 swap 为 y -> x，计算过程也大体相同。

### Band range price

对某个 band 来说，$p_{cu}$ 和 $p_{cd}$ 分别是这个 band 的上限价格和下限价格，当 AMM 内部价格到达 band 上限价格时，和 Uniswap V3 一样，band 内部 $y = 0$，当 AMM 价格到达 band 下限价格时，band 内部 $x = 0$。
将 $x = 0$ 代入上述恒等式中，可以求出 $y$，然后使用 AMM 价格公式，可以表示此时 AMM 价格，即为 band 的下限价格 $p_{cd}$ 的公式，同理也可以求出 band 的上限价格 $p_{cu}$ 的公式。这两个公式分别为：
...
上图横坐标为外部价格，纵坐标为 AMM 内价格，黑色实线表示一个 band 对应的参考外部上限界价格（$p_↑$ 和 $p_↓$），绿色实线表示此 band 的上下限价格（$p_{cu}$ 和 $p_{cd}$），蓝色虚线表示此时 AMM 价格 $p$ 等于外部价格 $p_o$，红色实现表示 AMM 内部价格。

我们可以发现，**随着外部价格增长，band 的上下限价格和 AMM 内部价格也以更快速度增长，且 AMM 内部价格始终处于 band 的上下限价格之内**。

### Band 之间的关系

LLAMMA 池在创建时需要设置好 A 值，同时还需要设定一个 $p_{base}$ 作为一个中心点价格。接下来就可以找出所有的 $p_↑$ 和 $p_↓$：

$$
p_↑(n) = (\frac{A - 1}{A})^np_{base},\ \ \ \ \ \ p_↓(n) = (\frac{A - 1}{A})^{n+1}p_{base}
$$

那么确定好 $p_o$ 的值之后，就可以知道每一个 band 中的上下限价格 $p_{cu}$ 和 $p_{cd}$ 了。

这幅图展示了三个 band 之间的关系（图片截取自 [desmos](https://www.desmos.com/calculator/60wavzznor)）：

![bands](../img/in-post/curve-stablecoin/bands.png)

上图中横坐标表示外部价格，纵坐标表示内部价格，蓝色虚线为斜率为 1 的直线。图中包含了 4 个预定义的参考外部上限界价格，他们之间组成了 3 个 band，每个 band 中上下限价格分别为绿色，橙色和紫色的曲线。实线表示此 band 预期会处于 active 状态，即可用来进行 swap（清算），虚线表示此 band 大概率在价外，处于未开始 swap 或者已经被全部 swap 完的状态。
...
当用户存入 ETH 铸造 crvUSD 时，Curve 需要帮助用户选择一组合适的 band，将用户的 ETH 存入这组 band 中，这组 band 需要能够满足：当用户 ETH 被全部转换成 crvUSD 时，得到的数量应该大于用户的债务（实际合约中还需乘以一个系数来留一些余地）。

那么我们就需要估算出，当外部价格降低，band 中 ETH 全部交易成 crvUSD 后得到的数量，我们将其记做 $x_↓$，另外我们还可以用同样方式估算，当外部价格反弹，band 中 crvUSD 全部交易成 ETH 后得到的数量，将其记做 $y_↑$.
...
这组公式使用的乐观估算的方式，即预估结果的最大值。前面我们说到外部价格的变化，会导致 LLAMMA 价格变化，产生和外部价格的价差，套利者的进入会导致用户产生损失，为了估算结果的最大值，我们需要假设：

- 外部价格以极其缓慢的速度变化，这意味着在一定时间内，外部价格和 LLAMMA 内部价格的价差极小，可以忽略
- 交易者在 LLAMMA 中交易，使得 LLAMMA 内部价格跟随外部价格同步变化，由于价差可以忽略，交易者者并不会获得价差带来的收益，因此它不能被视作套利者

这两个假定即 Curve 白皮书中所描述的 *adiabatically* 的含义，*adiabatically* 在物理学中表示绝热，在这里我们可以理解成隔绝了套利交易的一种理想状态。

满足这种理想状态时，LLAMMA 可以直接使用 Uniswap V3 的公式，计算出 swap 结果，即上述两个公式，公式的推导过程为：

![formula](../img/in-post/curve-stablecoin/formula.png)

有了清算/赎回结果估算，就可以知道用户的资产在被清算完成之后，是否足够偿付债务，如果不够，则需要清算者介入进行强制清算，具体过程和传统借贷清算过程类似，这里不再赘述。

当 $p = p_↑ = p_{cd}$ 时，$x = 0$ 因此，$x_↓ = y\sqrt{p_↓p_↑}$，即**理想情况下，用户资产被卖出的均价为用户 bands 的参考上下限价格的几何平均数：$sqrt{p_↓p_↑}$，同理用户资产被赎回时的价格也这个几何平均数**。
...
- LLAMMA Pool 中包含预定义的连续的 band
- band 由外部的参考价格 $p_↑$ 和 $p_↓$ 决定，他们之间的价格呈等比数列
- band 在 AMM 内部有 $p_{cu}$ 和 $p_{cd}$，表示 band 在 AMM 内的上下界价格
- LLAMMA AMM 价格和所有 band 的 $p_{cu}$ 和 $p_{cd}$ 都会随着外部价格 $p_o$ 的增大而增大，且变化速度更快，反之亦然
- 用户在创建债务的时候，资产将会被存入一组 band 中
- 当外部价格到达最大 band 的上限参考价格 $p_↑$ 时，此时理论上（假设存在足够多的套利者）也满足 $p = p_{cd} = p_↑$，用户的资产开始被清算
- 当外部价格到达最小 band 的下限参考价格 $p_↓$ 时，此时理论上满足 $p = p_{cu} = p_↓$，用户的资产被全部清算成 crvUSD

LLAMMA 只会对外暴露 swap 接口，而不会直接暴露添加/移除流动性的接口，因此用户在不创建债务的情况下时不能擅自添加流动性到 LLAMMA 中，这些相关操作有专门的 `Controller` 来负责。
...
其中第三个参数 band 的个数最少为 5 个，至多为 50 个。Controller 会根据这三个参数计算出用户需要存入的 band，并将用户的 ETH 存入到这些 band 中，同时将 crvUSD 发送给用户。

这个计算过程用到了前面提到的结果估算公式，在合约用对应函数 `Controller.create_loan()`，这里我不再详细解释这个函数的实现，如果感兴趣可以参考我在 Curve Discord 的[这个回复](https://discord.com/channels/729808684359876718/1044834395678310490/1048943587724906506)。

除此之外，Controller 还有增加抵押品，偿还债务，取出抵押品，强制清算，借贷利息维护等功能，这里就不一一赘述了。

## PegKeeper

PegKeeper 也叫做 stabilizer，是一组用来帮助 crvUSD 价格维持锚定的合约，这些合约主要用来和 Curve V1 pool 进行交互。

crvUSD 上线后，会在 Curve V1 中创建对应的池子。为了示例的简便，这里我们假设 crvUSD 和 DAI, USDT 各组一个池子。假设 crvUSD 的价格为 $p_s$：

- 当 $p_s > 1$ 时，意味着 crvUSD 供给不足，PegKeeper 会 mint 出 crvUSD 并添加到 Curve V1 池子中（添加 crvUSD 单币），以增加 crvUSD 的市场供给
- 当 $p_s < 1$ 时，意味着 crvUSD 供给过剩，PegKeeper 会将流动性从 Curve V1 池子中移除（移除 crvUSD 单币），以减少 crvUSD 的市场供给
...
当 $p_s > 1$ 时，则说明 Curve V1 池子中 crvUSD 不足，外部账户可以调用 PegKeeper 合约，mint 出 crvUSD，加入到 Curve V1 池子中（添加 crvUSD 单币），要注意这些 mint 出来的 crvUSD 是无抵押品的，因此只能被 PegKeeper 添加到 Curve V1 池子中，mint 的总数量会记录在 PegKeeper 合约的债务中。

当 $p_s$ 价格回稳或者 $p_s < 1$ 后，外部账户可以调用 PegKeeper 合约，从 Curve V1 池子中取出之前添加的 crvUSD（移除 crvUSD 单币）用于偿还之前的债务。
...
要注意的是，PegKeeper 合约内外部调用者并不能随意的调用 PegKeeper，合约内设置有一系列的检查，以保证外部调用者不会恶意调用，并且调用是能够产生利润的。

PegKeeper 的合约实现比较简单，有兴趣的读者可以自行研究。

## Monetary Policy

Curve Stablecoin 还设计了一个动态调节借贷利率的组件，用来根据市场供需调节利率，进一步帮助稳定 crvUSD 的价格锚定。
...
- 如果 crvUSD 价格大于 1，说明市场上 crvUSD 供给不足，完全可以靠 PegKeeper 凭空 mint 出 crvUSD 添加到 Curve v1 pool 中去补充市场上的 crvUSD 供给，此时保持低利率有助于吸引更多用户来铸造 crvUSD
- 如果 crvUSD 价格小于 1，说明市场上 crvUSD 供给过剩，PegKeeper 可以从 Curve V1 池子中移除 crvUSD 单币来减少 crvUSD 供给。但是有可能 PegKeeper 此前完全没有 mint 过 crvUSD，或者 PegKeeper 在偿还完它的所有债务后，crvUSD 价格仍然小于 1，此时出现巧妇难为无米之炊的情形，则必须靠升高利率逼迫用户偿还 crvUSD 债务，来减少市场供给。为了避免脱锚产生的价差过大， 利率升高的速度会在 crvUSD 低到某个临界点后急剧上升
...
A：用户只能通过 `Controller`，质押 ETH 创建 crvUSD 债务，ETH 会由 `Controller` 添加到 LLAMMA 中。普通用户也没有必要妄想在 LLAMMA 中做市，LLAMMA 的特性导致它的 LP 必定会在交易中亏损。
