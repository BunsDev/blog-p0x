...
Uniswap v3 解决这个问题的方式是，将 $[P_min, P_max]$ 这一段连续的价格范围为，分割成有限个离散的价格点。每两个相邻的价格区间称为一个 tick，用户在设置流动性的价格区间时，只能选择这些离散的价格点中的某一个作为流动性的边界价格。
...
如此一来 Uniswap v3 可以提供比较细粒度的价格选择范围（每个可选价格之间的差值为 0.01%），同时又可以将计算的复杂度控制在一定范围内。关于 tick 的管理实现方式，白皮书中进行了详尽的描述，本文不在赘述，后面章节也会对 tick 管理相关代码进行分析。
