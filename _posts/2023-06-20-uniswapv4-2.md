...
上文介绍了 Uniswap V4 中的 Hooks 特性，Uniswap V4 开源的 [V4-periphery](https://github.com/Uniswap/v4-periphery) 仓库中包含了几个 Hook 合约范例，其中大部分实现的功能比较简单，读者有兴趣可以自行阅读。本文将介绍一个比较复杂的 Hook 合约 [TWAMM.sol](https://github.com/Uniswap/v4-periphery/blob/e9ea3c9cb6caaa7b50f51a4e52073b60b8ccfe3a/contracts/hooks/examples/TWAMM.sol)，它实现了 TWAMM（Time-Weighted Average Market Maker）的功能。
...
当然，也可以让 TWAMM 每次用户与其交互时，自动结算 order pool，这样就省去了 keeper 的开销。

例如，在 ETH/USDC long order pool 中，需要交易的 token 速度为 100 USDC/s，那么有用户与 TWAMM 合约交互时，TWAMM 首先计算出距离上一次 long order pool 经过的时间，然后计算出需要交易的 token 数量，最后通过内置 AMM 进行相应的交易。

TWAMM 并没有限制内置 AMM 的类型，它可以是 Uniswap V2 类型的 AMM 也可以是 V4 这种集中流动性的 AMM. 在 paradigm 的论文里面，使用了 `x*y=k` AMM 作为范例。

## TWAMM 的数学计算

当某一个时间，TWAMM 需要结算 long/short 两个 order pool 中待交易的 token 时，会有一个问题：TWAMM 该先买还是先卖？

顺序的不同会导致最终的成交价格不同，例如，一个 `x*y=k` 的 AMM，假设 AMM 中余额 `ETH=1, USDC=1000, k=1000`，此时 long order pool 需要买入 500USDC, short order pool 2 需要卖出 1ETH. 来看看不同交易顺序对结果的影响：

- 假设先执行 pool1 中的交易，此时 AMM `ETH=0.6667, USDC=1500`，pool1 买入均价为 1500，接着执行 pool2 的交易，AMM 变成 `ETH=1.6667, USDC=599.98` ，pool2 卖出均价为 900.02
- 假设先执行 pool2 中的交易，此时 AMM `ETH=2, USDC=500`，pool2 卖出均价为 500，接着执行 pool1 的交易，AMM 变成 `ETH=1, USDC=1000`，pool1 买入均价为 500

很明显，后交易的 pool 一定会获得优势。

因此，需要找到一种更公平的交易方式，来结算两个 order pool 中的 token. 可以这样想象：

假设两个 order pool 同时开始交易，并且将交易的数量切成非常微小的份数，这样每次成交的价格都会非常接近 AMM 的价格，这样就可以保证两个 order pool 的成交价格都是公平的。
...
这样就得到了当 AMM 流动性不变时，结算完成后 AMM 的价格 $P_{ammEnd}$. 有了这个价格，我们就可以计算出对应的 $x_out$ 和 $y_out$ 了。
...
接下来考虑复杂一些的情形，假设 AMM 的流动性在 swap 过程中会发生变化，那么结算过程将需要变成一个迭代过程，用上面的公式计算流动性不变的区间内的结算结果，然后再计算下一个区间内的结算结果，直到结算完成。

现在假设 AMM 的初始流动性为 $L_1$, 通过前面的公式，计算出流动性不变的情况下，结算完成后 AMM 的价格 $P_{ammEnd}$. 但是当价格移动到 $P_a$($P_a < P_{ammEnd}$) 的时候，AMM 的流动性会发生改变。那么接下来需要知道的是，假设总共需要 swap 的 x token 数量为 $x_{in} = x_{rate} \cdot t$，那么需要分配多少 x token 到 $L_1$ 这个区间。

因为交易速率固定， token 的分配其实就是时间长度的分配，可以通过下面的公式计算这段时间：

$$
\sqrt{P_a} = \sqrt{\frac{y_{rate}}{x_{rate}}} \cdot \frac{e^{\frac{2t\sqrt{x_{rate}y_{rate}}}{L}} - c}{e^{\frac{2t\sqrt{x_{rate}y_{rate}}}{L}} + c}
$$

$$
\sqrt{\frac{y_{rate}}{x_{rate}}} \cdot (e^{\frac{2t\sqrt{x_{rate}y_{rate}}}{L}} - c) = \sqrt{P_a} \cdot (e^{\frac{2t_a\sqrt{x_{rate}y_{rate}}}{L}} + c)
$$

$$
\frac{2t\sqrt{x_{rate}y_{rate}}}{L} = \ln{(\frac{\sqrt{\frac{y_{rate}}{x_{rate}}} + \sqrt{P_a}}{\sqrt{\frac{y_{rate}}{x_{rate}}} - \sqrt{P_a}} \cdot c)}
$$

$$
t = \frac{\ln{(\frac{\sqrt{\frac{y_{rate}}{x_{rate}}} + \sqrt{P_a}}{\sqrt{\frac{y_{rate}}{x_{rate}}} - \sqrt{P_a}} \cdot \frac{\sqrt{\frac{y_{rate}}{x_{rate}}} - \sqrt{P_{ammStart}}}{\sqrt{\frac{y_{rate}}{x_{rate}}} + \sqrt{P_{ammStart}}})} \cdot L}{2\sqrt{x_{rate}y_{rate}}}
$$
...
接下来要做的就是 $x_{in} := x_{in} - x_{out}$, $y_{in} := y_{in} - y_{out}$, $L := L - \Delta L$, 进入下一个流动性区间，计算结算的结果。不断迭代直至结算 swap 完成。

最后，就可以计算出最终的 AMM 价格 $P_{ammEnd}$, 以及对应的 $x_{out}$ 和 $y_{out}$ 了。

那么如何将 $x_out$ 和 $y_out$ 分配给每个用户呢。这里使用了一种类似 Liquidity Mining 的累加算法，以 $y_out$ 为例，定义：

$$
earningsFactorPool0 = earningsFactorPool0 + \frac{y_{out}}{x_{rate}}
$$

假设用户的 x token 卖出速率为 $x_{rate1}$，那么用户得到的 y token 数量为：

$$
y = (earningsFactorPool0 - earningsFactorPool0Last) \cdot x_{rate1}
$$

TWAMM 核心的计算逻辑就是这样了，代码中还有很多实现细节，这里就不一一介绍了。
...
- 假设 token 价格保持不变，一直没有人在 pool 中交易，TWAMM 的 order pool 中累计了太多的待交易 token，下一次结算时会不会导致巨大的交易滑点？
...
- 如果有巨鲸提交大额的 TWAP Order，因为信息透明，会不会导致对市场产生影响，例如导致其他用户提前抢跑 TWAP Order 的交易？

因为 TWAP Order 是随时可以取消和修改数量的，巨鲸提交 TWAP Order 并不会有信息劣势，反而可能成为一个心理战术，其他用户抢跑交易也并一定会有利可图，因为 TWAP Order 随时可以取消。
