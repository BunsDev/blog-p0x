...
因为交易速率固定，token 数量的分配其实就是时间长度的分配，可以通过下面的公式计算这段时间：
...
$$
y_1 = (earningsFactorPool0_{current} - earningsFactorPool0_{last}) \cdot x_{rate1}
$$

这样就完成了 TWAMM 核心计算逻辑，代码中还有很多实现的细节，本文不再赘述。
