...
$$
L = \frac {\Delta x}{\frac 1{\sqrt {P_{b}}} - \frac 1{\sqrt {P_{c}}}} = \frac {\Delta y}{\sqrt {P_{c} - \sqrt {P_{a}}}}
$$

### 回调函数

使用回调函数原因是，将 `Position` 的 owner 和实际流动性 token 支付者解耦。这样可以中间合约来管理用户的流动性，并将流动性 token 化。关于 token 化，Uniswap v3 默认实现了 ERC721 token。其实对于单个池子来说，如果确定好流动性的价格边界，它也可以是 ERC20，这对挖矿合约来说具有参考意义（即挖矿合约需要自己管理一个 ERC20）。
...
tick 中和流动性相关的字段有两个 `liquidityGross`，`liquidityNet`。使用两个变量的原因是一个 tick 可能同时作为一个 postion 的下界和另一个 position 的上界。如果两个 position 中的流动性相等，例如 `L = 500`，而对于这个 tick `liquidityNet = 0`，此时我们需要一种机制来判断此 tick 是否还被引用，`liquidityGross = 1000` 我们可以知道这个 tick 仍然被引用。
