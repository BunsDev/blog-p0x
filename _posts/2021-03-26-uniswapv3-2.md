...
这里使用 `keccak256(abi.encode(token0, token1, fee)` 将 token0, token1, fee 作为输入，将哈希值作为 salt 来创建合约。这里实际上会使用 EVM 的 `CREATE2` 指令来创建合约。使用 `CREATE2` 指令的好处是，只要合约的 bytecode 及 salt 不变，那么创建出来的地址也将不变。
...
- 位图的保存模式是小端模式，即低地址位存储的 index 更小，高地址位存储的更大
...
这段代码的实现通过很多魔术的设置，优化了计算过程，其实现思路如下：
...
- 当 $i$ 的值为正数时，计算的结果可能会很大，中间计算的步骤可能会溢出，导致丢失最大位的精度，所以实际计算的是 $i$ 为负数时的值，因为这样丢失的是负数最小位的精度。即上面代码的那些魔术分别为 $\frac 1{\sqrt {1.10001}^1},\ \frac 1{\sqrt {1.10001}^2},\ \frac 1{\sqrt {1.10001}^3},\ ...,\ \frac 1{\sqrt {1.10001}^{20}}$ 的值
...
可以看到这个函数主要是将用户的 Position 保存起来，并给用户 NFT token，代表其所持有的流动性。至此提供流动性的步骤就完成了。移除流动性就是上述操作的逆操作，本文不再赘述了。
