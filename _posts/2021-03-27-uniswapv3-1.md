...
上面我们说了，通过引入虚拟 token 的概念，让用户可以在某一个价格区间内提供流动性。而每一个用户提供的流动性都可能设置不同的价格区间，这样一来一个交易对的池子中就包含了多个不同的流动性。因此从单个交易池的视角来看，Uni v3 实际上扮演的角色是一个交易聚合器：当发生交易时，此交易会拆分成多个，通过池中多个不同的流动性产生交易，最后聚合完成最终的交易过程。
...
因为每一次的价格变动都可能会引起流动性的变化，v3 需要围绕价格来进行交易相关的计算。对于一个流动性来说，当使用 x token 交换 y token 时，我们需要计算：
...
在实际交易过程中，一个交易会通过多个流动性聚合完成。因此上述的公式会进行聚合完成，即使用当前价格上的流动性总和来进行计算，流动性总和可以这样表示： $L_{total} = \sum L_{user}$.

一个交易同时还可能跨越不同的流动性阶段，因此合约需要维护每个用户提供流动性的价格边界，当价格到达边界时，需要在总流动性上增加或移除对应流动性大小。通过分段计算的方式完成交易结果的计算，具体的实现过程会在后面的代码分析中讲解。
