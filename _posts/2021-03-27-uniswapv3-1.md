...
白皮书中已经详细解释了 tick 作用。简单说，一个 tick 就代表 Uniswap 价格的等比数列中的某一个价格，因此每一个用户提供的流动性的价格边界可以用 $tick_{lower}$ 和 $tick_{upper}$ 来表示。为了记录的计算，对于每一个交易对，都定义一个价格为 1 个 tick。并且将所有 tick 通过索引来表示，定义整数 $i$ 表示 tick 的索引：
